					  ...
					  int *master_id, char *master_conninfo_out)
{
	...
	for (i = 0; i < PQntuples(res); i++)
	{
		...
		log_verbose(LOG_INFO,
					_("checking role of cluster node '%i'， remote conninfo is '%s'\n"),
					node_id, remote_conninfo);
...
#ifdef ADB
int 
get_node_record_by_node_id(PGconn *conn, char *cluster, int node_id, t_node_info *node_info)
{
	char		sqlquery[QUERY_STR_LEN];
	int result;
	...
	//log_verbose(LOG_DEBUG, "get_node_record_by_node_id():\n%s\n", sqlquery);
	log_notice("get_node_record_by_node_id():\n%s\n", sqlquery);

	result = _get_node_record(conn, cluster, sqlquery, node_info);

	if (result == 0)
	{
		log_verbose(LOG_DEBUG, "get_node_record(): no record found for node %d\n", node_id);
	}

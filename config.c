...
_parse_config(t_configuration_options *options, ItemList *error_list)
{
...
#ifdef ADB
	memset(options->regist_conninfo, 0, sizeof(options->regist_conninfo));
	memset(options->group_name, 0, sizeof(options->group_name));
	options->is_agtm = IS_NOT_AGTM_SERVER;
#endif	
...
#ifdef ADB
		else if (strcmp(name, "regist_conninfo") == 0)
			strncpy(options->regist_conninfo, value, MAXLEN);
		else if (strcmp(name, "group_name") == 0)
			strncpy(options->group_name, value, MAXLEN);	
		else if (strcmp(name, "is_agtm") == 0)
			options->is_agtm = repmgr_atoi(value, "is_agtm", error_list, false);
#endif			
	...
	if (strlen(options->conninfo))
	{
		...
		log_notice(_("options->conninfo: \"%s\"\n"), options->conninfo); 
	...
	if (strlen(options->regist_conninfo))
	{
		...
		/* Sanity check the provided regist_conninfo string
		 *
		 * NOTE: PQconninfoParse() verifies the string format and checks for valid options
		 * but does not sanity check values
		 */
		
		log_notice(_("options->regist_conninfo: \"%s\"\n"), options->regist_conninfo); 
		conninfo_options = PQconninfoParse(options->regist_conninfo, &conninfo_errmsg);
		if (conninfo_options == NULL)
		{
			char	   error_message_buf[MAXLEN] = "";
			snprintf(error_message_buf,

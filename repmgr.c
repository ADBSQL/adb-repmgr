...
main(int argc, char **argv)
{
	...
	{	
		...
		maxlen_snprintf(repmgr_schema, "%s%s_%s", DEFAULT_REPMGR_SCHEMA_PREFIX,
						repmgr_cluster, options.group_name);
#else		
...
#ifdef ADB
		maxlen_snprintf(repmgr_schema, "%s%s_%s", DEFAULT_REPMGR_SCHEMA_PREFIX,
						options.cluster_name, options.group_name);
#else		
		maxlen_snprintf(repmgr_schema, "%s%s", DEFAULT_REPMGR_SCHEMA_PREFIX,
						options.cluster_name);
#endif
	}					
	log_notice(_("repmgr_schema: \"%s\"\n"), repmgr_schema); 
	/*
...
#ifdef ADB
	if (strlen(options.regist_conninfo))
	{
		conn = establish_db_connection(options.regist_conninfo, true);
	}	
#else
...
do_master_register(void)
{
	...
	*/
	...
	log_info(_("primary node id is (%d)\n"), primary_node_id);
	...
	else
	{
		...
		log_info(_("create node record for master...\n"));
...
do_standby_register(void)
{
...
#ifdef ADB
	log_info(_("connecting to register server database\n"));
	conn = establish_db_connection_quiet(options.regist_conninfo);
#else
...
#ifdef ADB
		standby_conn = establish_db_connection_quiet(options.conninfo);
		ret = is_standby(standby_conn);
#else
		ret = is_standby(conn);
#endif
	...
	log_notice(_("Check if node record exists for node: %i\n"),options.node);
	
#ifdef ADB
	node_result = get_node_record_by_node_id(conn,
										  options.cluster_name,
										  options.node,
										  &node_record);
#else
	node_result = get_node_record_by_name(master_conn,
										  ...
										  &node_record);
#endif
	log_notice(_("Check node_result values : %i\n"),node_result);
...
//	conn = establish_db_connection(options.conninfo, true);
	...
	/* Check node exists and is really a standby */
		...
		get_conn = conn;
#else
		get_conn = master_conn;
#endif	

	if (!get_node_record(get_conn, options.cluster_name, target_node_id, &node_info))
	{
		log_err(_("No record found for node %i\n"), target_node_id);
		exit(ERR_BAD_CONFIG);
	}
	
	//log_notice(_("node_info.type is :%s\n"),node_info.type);
	...
	log_info(_("unregistering the standby\n"));
		...
		del_conn = conn;
#else
		del_conn = master_conn;
#endif	
	node_record_deleted = delete_node_record(del_conn,
										     target_node_id,
											 "standby unregister");
	...
	/* Log the event */
		...
		cre_conn = conn;
#else
		cre_conn = master_conn;
#endif		
	create_event_record(cre_conn,
						&options,
						target_node_id,
						"standby_unregister",
						true,
...
do_standby_promote(void)
{
...
#ifdef ADB
	log_info(_("connecting to regist server\n"));
	regist_conn = establish_db_connection(options.regist_conninfo, true);
#endif	
	...
	{
		log_notice(_("try reconnect to the new master, trying to reconnect... %i seconds after promote decision\n"), i);
		retval = is_standby(conn);
		if (!retval)
		{
	...
	PQfinish(regist_conn);
	do
	{
		log_notice(_("waiting the new master connection availability\n"));
		if (is_pgup(conn, options.master_response_timeout))
		{
			regist_conn = establish_db_connection(options.regist_conninfo, true);
			if(PQstatus(regist_conn) != CONNECTION_OK )
			{
				sleep(1);
			}
			else
	...
	if ( update_node_record_set_master(regist_conn, options.node) == false )
#else	
	...
	/* Log the event */
#ifdef ADB
	create_event_record(regist_conn,
#else	
	create_event_record(conn,
#endif	
						&options,
...
create_schema(PGconn *conn)
{
	...
	if(options.is_agtm == IS_NOT_AGTM_SERVER)
	{
		sqlquery_snprintf(sqlquery,
					  ...
					  "  apply_lag                      BIGINT NOT NULL) TO NODE(%s)",
					  get_repmgr_schema_quoted(conn), options.node_name);
	}
	else
	{
